...
const { analyzeChatWithOpenAI, handleChatRequest, applyPlanUpdate } = require('./index');

// Copy-specific provider defaults: allow Qwen config without touching the original server.js
if (!process.env.LLM_PROVIDER && process.env.QWEN_API_KEY) {
  process.env.LLM_PROVIDER = 'qwen';
}
if (!process.env.LLM_API_KEY && process.env.QWEN_API_KEY) {
  process.env.LLM_API_KEY = process.env.QWEN_API_KEY;
}
if (!process.env.LLM_MODEL && process.env.LLM_PROVIDER === 'qwen') {
  process.env.LLM_MODEL = 'qwen-plus';
}
if (!process.env.LLM_BASE_URL && process.env.LLM_PROVIDER === 'qwen') {
...
async function handleChatbotRequest(req, res) {
  try {
    ...
    return sendJson(res, 200, {
      source: process.env.LLM_PROVIDER ? `${process.env.LLM_PROVIDER}-rag` : (process.env.OPENAI_API_KEY ? 'openai-rag' : 'local-fallback'),
      analysis,
      updatedItinerary
    });
  } catch (error) {
